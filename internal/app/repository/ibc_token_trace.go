...
type ITokenTraceRepo interface {
	...
	UpdateBaseDenomAndChain(bulk *qmgo.Bulk, chain, denom, baseDenom, baseDenomChain string) error
	FindInvalidDenomValue(skip, limit int64) ([]*entity.IBCTokenTrace, error)
	Bulk() *qmgo.Bulk
}
...
func (repo *TokenTraceRepo) Bulk() *qmgo.Bulk {
...
func (repo *TokenTraceRepo) UpdateBaseDenomAndChain(bulk *qmgo.Bulk, chain, denom, baseDenom, baseDenomChain string) error {
	query := bson.M{
		"chain": chain,
		"denom": denom,
	}
	bulk.UpdateOne(query, bson.M{
		operator.Set: bson.M{
			"base_denom":       baseDenom,
			"base_denom_chain": baseDenomChain,
		},
...
func (repo *TokenTraceRepo) FindInvalidDenomValue(skip, limit int64) ([]*entity.IBCTokenTrace, error) {
	q := bson.M{
		"denom_value": "0",
	}
	var res []*entity.IBCTokenTrace
	err := repo.coll().Find(context.Background(), q).Limit(limit).Skip(skip).All(&res)
	return res, err
}
