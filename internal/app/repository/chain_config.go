...
type IChainConfigRepo interface {
	...
	FindAllOnlyLcdInfo() ([]*entity.ChainConfig, error)
	FindHomeChainInfos() ([]*entity.ChainConfig, error)
	...
	UpdateIbcInfo(bulk *qmgo.Bulk, config *entity.ChainConfig) error
...
func (repo *ChainConfigRepo) Bulk() *qmgo.Bulk {
...
func (repo *ChainConfigRepo) FindAll() ([]*entity.ChainConfig, error) {
	...
	err := repo.coll().Find(context.Background(), bson.M{"status": entity.ChainStatusOpen}).All(&res)
	return res, err
}
func (repo *ChainConfigRepo) FindAllOnlyLcdInfo() ([]*entity.ChainConfig, error) {
	var res []*entity.ChainConfig
	err := repo.coll().Find(context.Background(), bson.M{"status": entity.ChainStatusOpen}).
		Select(bson.M{ChainConfigFieldChainName: 1, ChainConfigFieldLcdApiPath: 1, ChainConfigFieldGrpcRestGateway: 1}).All(&res)
	return res, err
}

func (repo *ChainConfigRepo) FindHomeChainInfos() ([]*entity.ChainConfig, error) {
	var res []*entity.ChainConfig
...
func (repo *ChainConfigRepo) UpdateIbcInfo(bulk *qmgo.Bulk, config *entity.ChainConfig) error {
	bulk.UpdateOne(bson.M{ChainConfigFieldChainName: config.ChainName}, bson.M{
		"$set": bson.M{
