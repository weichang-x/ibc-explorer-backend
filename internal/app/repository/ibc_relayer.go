...
type IRelayerRepo interface {
	...
	UpdateRelayerTime(bulk *qmgo.Bulk, relayerId string, updateTime int64) error
	UpdateTxsInfo(bulk *qmgo.Bulk, relayerId string, txs, txsSuccess int64, totalValue, totalFeeValue string) error
	...
	UpdateChannelPairInfo(bulk *qmgo.Bulk, relayerId string, infos entity.ChannelPairInfoList) error
	Update(bulk *qmgo.Bulk, relayer *entity.IBCRelayerNew) error
...
func (repo *IbcRelayerRepo) Bulk() *qmgo.Bulk {
	return repo.coll().Bulk()
}
func (repo *IbcRelayerRepo) Update(bulk *qmgo.Bulk, relayer *entity.IBCRelayerNew) error {
	updateData := bson.M{
	...
	bulk.UpdateOne(bson.M{RelayerFieldelayerId: relayer.RelayerId}, bson.M{
		"$set": updateData})
	return nil
}
...
func (repo *IbcRelayerRepo) UpdateChannelPairInfo(bulk *qmgo.Bulk, relayerId string, infos entity.ChannelPairInfoList) error {
	if len(infos) == 0 {
		return nil
	}
	...
	updateData := bson.M{
		...
		RelayerFieldServedChains:    len(infos.GetChains(nil)),
	}

	bulk.UpdateOne(bson.M{RelayerFieldelayerId: relayerId}, bson.M{
		"$set": updateData})
	return nil
}

func (repo *IbcRelayerRepo) UpdateTxsInfo(bulk *qmgo.Bulk, relayerId string, txs, txsSuccess int64, totalValue, totalFeeValue string) error {
	updateData := bson.M{
		RelayerFieldUpdateAt: time.Now().Unix(),
	}
	if totalValue != "" {
...
func (repo *IbcRelayerRepo) UpdateRelayerTime(bulk *qmgo.Bulk, relayerId string, updateTime int64) error {
	bulk.UpdateOne(bson.M{RelayerFieldelayerId: relayerId}, bson.M{
		"$set": bson.M{
			RelayerFieldUpdateAt:   time.Now().Unix(),
