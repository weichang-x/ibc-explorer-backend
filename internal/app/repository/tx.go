...
type ITxRepo interface {
	...
	GetLatestAcknowledgeTx(chain, packetId string) (*entity.Tx, error)
	GetLatestRecvPacketTx(chain, packetId, dcConnectionId string) (*entity.Tx, error)
...
func (repo *TxRepo) GetFirstTx(chain string) (*entity.Tx, error) {
	...
	defer cancel()
	err := repo.coll(chain).Find(ctx, bson.M{}).Sort("time").One(&res)
	return &res, err
}
...
func (repo *TxRepo) GetUpdateTimeByUpdateClient(chain, address, clientId string, startTime int64) (int64, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, query).
...
func (repo *TxRepo) GetTransferTx(chain string, height, limit int64) ([]*entity.Tx, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, query).Sort("height").Limit(limit).All(&res)
	return res, err
}
...
func (repo *TxRepo) GetTxByHash(chain string, hash string) (entity.Tx, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, bson.M{"tx_hash": hash}).Sort("-height").One(&res)
	return res, err
}
func (repo *TxRepo) GetTxByHashes(chain string, hashs []string) ([]*entity.Tx, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, bson.M{"tx_hash": bson.M{
		"$in": hashs,
	}}).Sort("height").All(&res)
	return res, err
}

func (repo *TxRepo) GetLatestAcknowledgeTx(chain, packetId string) (*entity.Tx, error) {
	var res *entity.Tx
	query := bson.M{
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, query).Select(bson.M{"events_new": 0}).Sort("-height").One(&res)
...
func (repo *TxRepo) GetLatestRecvPacketTx(chain, packetId, dcConnectionId string) (*entity.Tx, error) {
	var res *entity.Tx
	query := bson.M{
		"msgs.type":          constant.MsgTypeRecvPacket,
		"msgs.msg.packet_id": packetId,
	...
	ignoreField := bson.M{}
	if dcConnectionId != "" {
		ignoreField = bson.M{"events_new": 0}
	}
	err := repo.coll(chain).Find(ctx, query).Select(ignoreField).Sort("-height").One(&res)
	return res, err
}
...
func (repo *TxRepo) RelayerDenomStatistics(chain string, startTime, endTime int64) ([]*dto.RelayerDenomStatisticsDTO, error) {
	...
	defer cancel()
	err := repo.coll(chain).Aggregate(ctx, pipe).All(&res)
	return res, err
}
...
func (repo *TxRepo) GetRelayerTxs(chain string, relayerAddrs []string, txTypes []string,
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, query).Sort("-time").Hint(GetRelayerTxsHintIndexName()).Skip(skip).Limit(limit).All(&res)
	return res, err
}
...
func (repo *TxRepo) CountRelayerTxs(chain string, relayerAddrs []string, txTypes []string, txTimeStart, txTimeEnd int64) (int64, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	return repo.coll(chain).Find(ctx, query).Hint(CountRelayerTxsHintIndexName()).Count()
}
...
func (repo *TxRepo) GetAddressTxs(chain, address string, skip, limit int64) ([]*entity.Tx, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, query).Sort("-time").Skip(skip).Limit(limit).All(&res)
	return res, err
}
...
func (repo *TxRepo) CountAddressTxs(chain, address string) (int64, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	return repo.coll(chain).Find(ctx, query).Count()
}
...
func (repo *TxRepo) GetAddressLatestTx(chain, address string) (*entity.Tx, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), constant.QueryTimeout*time.Second)
	defer cancel()
	err := repo.coll(chain).Find(ctx, query).Sort("-time").One(&res)
	return &res, err
}
