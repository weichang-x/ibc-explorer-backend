...
func (svc HomeService) ChainsConnection() (vo.ChainsConnectionResp, errors.Error) {
	...
	for i := range chainCfgs {
		item := vo.IobChainDto{
			ChainName:      chainCfgs[i].ChainName,
			PrettyName:     chainCfgs[i].PrettyName,
			CurrentChainId: chainCfgs[i].CurrentChainId,
			Icon:           fmt.Sprintf(constant.IBCConnectionChainsIconUri, chainCfgs[i].ChainName),
		}
		connectionChains := make([]vo.IobConnectionChain, 0, len(chainCfgs[i].IbcInfo))
...
func (svc HomeService) DailyChains() (vo.DailyChainsResp, errors.Error) {
	...
	chainCfgs, err := chainCfgRepo.FindHomeChainInfos()
	if err != nil {
		return resp, errors.Wrap(err)
	}
