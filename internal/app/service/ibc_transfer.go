...
type ITransferService interface {
	...
	TransferTxsVolume(req *vo.TranaferTxsValueReq) (*vo.TransferTxsValueResp, errors.Error)
...
func (t TransferService) TransferTxsCount(req *vo.TranaferTxsReq) (*vo.TransferTxsCountResp, errors.Error) {
	...
	if len(query.Chain) == 0 && len(query.Status) == 4 && query.StartTime == 0 && len(query.BaseDenom) == 0 && query.Denom == "" {
		...
		return &vo.TransferTxsCountResp{
			TxsCount: checkDisplayMax(data.Count),
		}, nil
	}
	...
	return &vo.TransferTxsCountResp{
		TxsCount: checkDisplayMax(count),
	}, nil
}

func (t TransferService) TransferTxsVolume(req *vo.TranaferTxsValueReq) (*vo.TransferTxsValueResp, errors.Error) {
	query, err := createIbcTxQuery(&vo.TranaferTxsReq{
		DateRange:      req.DateRange,
		Status:         req.Status,
		Chain:          req.Chain,
		Denom:          req.Denom,
		BaseDenom:      req.BaseDenom,
		BaseDenomChain: req.BaseDenomChain,
	})
	if err != nil {
		return nil, errors.Wrap(err)
	}
	...
	if (len(query.Chain) == 0) ||
		...
		return &vo.TransferTxsValueResp{}, nil
	}

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	aggrTxsValue, err2 := ibcTxRepo.AggrTxsValue(ctx, query, false)
	if err2 != nil {
		return &vo.TransferTxsValueResp{}, errors.Wrap(err2)
	}
	...
	return &vo.TransferTxsValueResp{
		TxsValue: totalTxsValue.String(),
	}, nil
}
