...
func (svc *RelayerService) Detail(relayerId string) (vo.RelayerDetailResp, errors.Error) {
	...
	//chains, err := chainCache.FindAll()
	//if err != nil {
	//	return resp, errors.Wrap(err)
	//}
	//supportChainMap := make(map[string]struct{}, len(chains))
	//for i := range chains {
	//	supportChainMap[chains[i].Chain] = struct{}{}
	//}
	...
	servedChainsInfo := entity.ChannelPairInfoList(one.ChannelPairInfo).GetChains(nil)
	resp = vo.LoadRelayerDetailDto(one, channelPairStatusMap, servedChainsInfo)
