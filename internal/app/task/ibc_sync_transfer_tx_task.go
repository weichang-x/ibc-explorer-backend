...
type syncTransferTxWorker struct {
	...
	addrPrefixChainMap map[string]string
}

func (w *syncTransferTxWorker) initPrefixMap() {
	w.addrPrefixChainMap = make(map[string]string, len(w.chainMap))
	for _, val := range w.chainMap {
		w.addrPrefixChainMap[val.AddrPrefix] = val.ChainName
	}
}
...
func (w *syncTransferTxWorker) handleSourceTx(chain string, txs []*entity.Tx, denomMap map[string]*entity.IBCDenom) ([]*entity.ExIbcTx, entity.IBCDenomList) {
	var ibcTxList []*entity.ExIbcTx
	var ibcDenomList entity.IBCDenomList
	for i := range txs {
		for msgIndex, msg := range txs[i].DocTxMsgs {
			...
			var fullDenomPath, sequence, scConnection, dcChainAddrPrefix string
			ibcDenom, isExisted := denomMap[scDenom]
			if ibcTxStatus != entity.IbcTxStatusFailed {
				dcPort, dcChannel, fullDenomPath, sequence, scConnection, dcChainAddrPrefix = parseTransferTxEvents(msgIndex, txs[i])
				if !isExisted {
					ibcDenom = ibctool.TraceDenom(fullDenomPath, chain, w.chainMap)
				}
			}

			if dcChainAddrPrefix != "" && dcChain == "" {
				dcChain = w.addrPrefixChainMap[dcChainAddrPrefix]
			}
			...
			recordIdStr := fmt.Sprintf("%s%s%s%s%s%s%s%d", scPort, scChannel, dcPort, dcChannel, sequence, chain, txs[i].TxHash, msgIndex)
...
func (w *syncTransferTxWorker) getTxList(chain string, height, limit int64) ([]*entity.Tx, error) {
	...
	txs, err := txRepo.FindByTypeAndHeight(chain, constant.MsgTypeTransfer, maxHeight)
	if err != nil {
	...
	for i := range txs {
		_, ok := txHashMap[txs[i].TxHash]
		if !ok {
			transferTxList = append(transferTxList, txs[i])
		}
	}
