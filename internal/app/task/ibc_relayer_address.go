...
func (t *RelayerAddressGatherTask) repairEmptyPubKey() {
	...
	fastFailChainMap := make(map[string]string)
	rlyAddrBulk := relayerAddressRepo.Bulk()
	var change bool
	for _, v := range addresses {
		...
		if err = relayerAddressRepo.UpdatePubKey(rlyAddrBulk, v.Address, v.Chain, account.Account.PubKey.Key); err != nil {
			logrus.Errorf("task %s UpdatePubKey err, %v", t.Name(), err)
		}
	}
	if change {
		rlyAddrBulk.SetOrdered(false)
		if _, err := rlyAddrBulk.Run(context.Background()); err != nil {
			logrus.Errorf("task %s bulk UpdatePubKey err, %v", t.Name(), err)
		}
	}
...
func (t *RelayerAddressGatherTask) gather() {
	...
	for _, v := range todoList {
		...
		if registeredRelayer != nil {
			if err1 := t.addRelayerChannelPair(rlyBulk, rlyAddrBulk, registeredRelayer, v.Address, v.Chain, entity.GatherStatusRegistry); err1 != nil {
				continue
			}
			change = true
		} else if unknownRelayer != nil {
			if err1 := t.addRelayerChannelPair(rlyBulk, rlyAddrBulk, unknownRelayer, v.Address, v.Chain, entity.GatherStatusPubKey); err1 != nil {
				continue
			}
			change = true
		} else {
			if err1 := t.addRelayer(rlyBulk, rlyAddrBulk, v.Address, v.Chain); err1 != nil {
				continue
			}
			change = true
		}
	...
	if change {
		...
		go func() {
			...
			if _, err := rlyBulk.Run(context.Background()); err != nil && err != mongo.ErrNoDocuments {
				logrus.Errorf("task %s bulk update RelayerChannelPair error, %s", t.Name(), err.Error())
			}
		}()
		go func() {
			defer wg.Done()
			if _, err := rlyAddrBulk.Run(context.Background()); err != nil && err != mongo.ErrNoDocuments {
				logrus.Errorf("task %s bulk update UpdateGatherStatus error, %s", t.Name(), err.Error())
			}
		}()
		wg.Wait()
	}
...
func (t *RelayerAddressGatherTask) addRelayerChannelPair(relyBulk, relyAddrBulk *qmgo.Bulk, relayer *entity.IBCRelayerNew, address, chain string, status entity.GatherStatus) error {
	...
	relayerRepo.UpdateChannelPairInfo(relyBulk, relayer.RelayerId, relayer.ChannelPairInfo)
	relayerAddressRepo.UpdateGatherStatus(relyAddrBulk, address, chain, status)
	return nil
}

func (t *RelayerAddressGatherTask) addRelayer(relyBulk, relyAddrBulk *qmgo.Bulk, address, chain string) error {
	newPair, err := t.getNewPair(address, chain)
	if err != nil {
		return err
	}
	...
	//}
	relyBulk.InsertOne(newRelayer)

	relayerAddressRepo.UpdateGatherStatus(relyAddrBulk, address, chain, entity.GatherStatusPubKey)
	return err
}
