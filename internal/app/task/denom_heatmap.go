...
func (t *DenomHeatmapTask) init() error {
	...
	allChainList, err := chainConfigRepo.FindAllOnlyLcdInfo()
	if err != nil {
		logrus.Errorf("task %s FindAllOnlyLcdInfo error, %v", t.Name(), err)
		return err
	}

	t.chainConfigMap = make(map[string]*entity.ChainConfig, len(allChainList))
	for _, v := range allChainList {
		t.chainConfigMap[v.ChainName] = v
	}
...
func (t *DenomHeatmapTask) getSupplyFromLcd(chainCfg *entity.ChainConfig) {
	...
	for {
		...
		bz, err := utils.HttpGet(url)
		if err != nil {
			logrus.Errorf("task %s chain: %s setSupply url:%s error, %v", t.Name(), chain, url, err)
			return
		}
...
func (t *IBCDenomHopsTask) Run() int {
	...
	for {
		...
		for _, v := range denomList {
			...
			denomRepo.UpdateHops(bulk, v.Chain, v.Denom, hops)
		}

		bulk.SetOrdered(false)
		if _, err := bulk.Run(context.Background()); err != nil {
			logrus.Errorf("task %s UpdateHops error, %s", t.Name(), err.Error())
		}
